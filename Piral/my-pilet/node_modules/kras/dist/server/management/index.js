...
function withManagement(server, config) {
    ...
    if (api !== false) {
        ...
        protect(server.at(api, 'config'))
            .get(basics_1.configOf(server, config))
            .put(basics_1.updateClient(server, config));
        protect(server.at(api, 'logs'))
            .get(logs_1.recentLogsOf(server))
            .delete(logs_1.allLogsOf(server))
            .feed(logs_1.liveLogs(server));
        protect(server.at(api, 'broadcast')).post(broadcast_1.broadcastAt(server));
        protect(server.at(api, 'data'))
            .get(overview_1.overview(server))
            .feed(overview_1.liveData(server));
        ...
        protect(server.at(api, 'file', ':name'))
            .get(files_1.readFile(server))
            .put(files_1.saveFile(server));
        protect(server.at(api, 'settings'))
            .get(settings_1.readSettings(server))
            .put(settings_1.saveSettings(server));
        ...
        protect(server.at(api, 'injector', ':name'))
            .get(injectors_1.readInjectorSettings(server))
            .put(injectors_1.saveInjectorSettings(server));
    }
}
